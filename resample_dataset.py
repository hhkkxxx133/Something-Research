...
def load_labels_from_bin(binfile, mode):
    ...
    l = {}
    haveread = 0
    # val = [411, 413, 415, 417, 419, 681, 685, 687, 689] # 685
    # test = [293, 324, 664, 714, 767, 864, 873, 1159, 1209, 1255] # 664
    for idx, n in enumerate(num_frames):
        if idx+1 in interest[mode]:
            l[mode+'_'+str(idx+1).zfill(7)] = labels[haveread:haveread+n]
        haveread += n
    return l, list(num_frames)

def load_inputs(labels, frm_path, mode):
    dataset = []
    ...
    for video in vidlist:
        # skip all the videos that don't belong to 20 classes
        ...
        for frameid in range(len(labellist)):
            dataset.append( {'video':video, 'frame':frameid+1, 'label':int(labellist[frameid])} )
            # dataset.append({'dir':os.path.join(frm_path, video), 'frame':frameid+1, 'label':int(labellist[frameid])})
    return dataset
...
class MyDataset(data.Dataset):
    ...
    Argument
    """
    def __init__(self, mode, frm_path, pose_path, binfile, transform=None):
        ...
        self.dataset = load_inputs(self.labels, self.frm_path, self.mode)


    def split_dataset(self):
        self.pool = {}
        for k,v in gtclasses.items(): # 0:'background'
            self.pool[v] = [0, []]

        for v in self.dataset:
            self.pool[ gtclasses[v['label']] ][1].append(v)
    ...
    def get_train_data(self, index):
        ...
        idx = index % len(gtclasses)
        v = self.pool[ gtclasses[idx] ][1] [ self.pool[gtclassesl[idx]] [0] ]
        videoid = v['video']
        frameid = v['frame']
        label = v['label']
        self.pool[gtclasses[idx]][0] += 1
        if self.pool[gtclasses[idx]][0] >= len(self.pool[gtclasses[idx]][1]):
            self.pool[gtclasses[idx]][0] = 0
